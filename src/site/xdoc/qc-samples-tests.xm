  ...
  <body>
    ...
    <section name="Quality control samples tests">
     ...
     You will find below all the parameters for all the available tests in Aozan.</p>
       ...
       <p> Aozan launch FastQC on each sample (using parallel computation) and creates a report file including the thresholds fixed for quality controls.
    ...
    <div align="center">
     ...
     <p>Uses the principles from FastQ Screen. 
    	...
    	Creates a report file with values for each genome
			...
			This solution requires to have a lot of free space in temporary directory used. 
			...
			<table>
			  <tr><th>Aozan property</th><th>Type</th><th>description</th></tr>
			  <tr><td>qc.conf.genome.alias.path</td><td>string</td>
				  <td>Path to the file which make the correspondance between genome name in casava design file and the genome name reference used for
			  ...
			  <tr><td>qc.conf.settings.genomes.desc.path</td><td>string</td>
				  <td>Path to the genomes descriptions repository. The genome description file contains some basic informations about the genome
						...
						previous run. </td></tr>
			  <tr><td>qc.conf.settings.genomes</td><td>string</td><td>Path to the genomes repository.</td></tr>
 			  <tr><td>qc.conf.settings.mappers.indexes.path</td><td>string</td><td>Path to the genomes indexes repository.</td></tr>
			  <tr><td>qc.conf.fastqscreen.genomes</td><td>string</td><td>List of reference genomes used per default for fastqscreen (e.g. phix, adapters)</td></tr>
			</table>
