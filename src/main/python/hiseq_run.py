...
def get_runinfos_file(run_id, conf):
    ...
    Arguments:
        ...
        conf: configuration dictionary
    """
    common.log("debug", "get_runinfos_file, run_id: " + run_id, conf)
    sequencer_path = find_hiseq_run_path(run_id, conf)
    common.log("debug", "get_runinfos_file, sequencer_path: " + sequencer_path, conf)
    if type(sequencer_path) is bool:
        return None

    path = sequencer_path + '/' + run_id + '/RunInfo.xml'
    common.log("debug", "get_runinfos_file, path: " + path, conf)
    common.log("debug", "get_runinfos_file, os.path.exists(path): " + str(os.path.exists(path)), conf)
...
def get_run_info(run_id, conf):
    ...
    common.log("debug", "get_run_info, file_src: " + str(file_src), conf)
    if file_src == None:
        return None
...
def get_read_count(run_id, conf):
    ...
    common.log("debug", "get_read_count, run_info: " + str(run_info), conf)
    if run_info == None:
        return -1
    common.log("debug", "get_read_count, run_info.getReads().size(): " + str(run_info.getReads().size()), conf)
...
def get_lane_count(run_id, conf):
    ...
    run_info = get_run_info(run_id, conf)
    common.log("debug", "get_lane_count, run_info: " + str(run_info), conf)
    if run_info == None:
        return -1
    common.log("debug", "get_lane_count, run_info.getFlowCellLaneCount(): " + str(run_info.getFlowCellLaneCount()), conf)
