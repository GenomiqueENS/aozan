...
def send_report(run_id, conf):
    ...
    common.send_msg_with_attachment('[Aozan] First base report for run ' + type_run_estimated + '  ' + run_id + ' on '
                                    + common.get_instrument_name(run_id, conf), message, attachment_file, conf)
