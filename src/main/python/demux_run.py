...
def get_bcl2fastq_mismatches(samplesheet, default_value, run_id, conf):
    ...
    for value in list:
        ...
        if len(result) == 0:
            error('Error while reading Bcl2fastq samplesheet for run ' + run_id,
                  'No number of allowed mismatches found in samplesheet for run ' + run_id + '.', conf)
            return False
...
def demux_run_with_docker(run_id, input_run_data_path, fastq_output_dir, samplesheet_csv_path, nb_mismatch, conf):
    ...
    try:
        ...
        docker = DockerCommand(conf[Settings.DOCKER_URI_KEY], ['/bin/bash', '-c', cmd], 'genomicpariscentre/bcl2fastq2:' + bcl2fastq_version)
